...
class EnrollmentsRepositoryImpl implements EnrollmentsRepository {
  ...
  @override
  Future<Result<TodayTasksEntity>> getTodayTasks(String enrollmentId) async {
    try {
      return Result.success(await _dataSource.getTodayTasks(enrollmentId));
    } on AppException catch (e) {
      return Result.failure(e.toFailure());
    } catch (e) {
      return Result.failure(UnknownFailure(e.toString()));
  ...
  Future<Result<void>> useFreeze(String enrollmentId) async {
    try {
      await _dataSource.useFreeze(enrollmentId);
