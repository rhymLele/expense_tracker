...
class _TodayTasks extends StatelessWidget {
  ...
  const _TodayTasks(
      {required this.tasks, required this.completedIds, required this.enrollment});
...
class _TaskCard extends StatelessWidget {
  ...
  final bool isDone;
  final EnrollmentEntity enrollment;
  const _TaskCard({required this.task, required this.isDone, required this.enrollment});

  static const _exerciseTypes = {'quiz', 'mcq', 'fill_blank', 'matching', 'ordering', 'multi_select'};

  @override
  Widget build(BuildContext context) {
    final cubit = context.read<RoadmapHomeCubit>();
    return GestureDetector(
      onTap: () async {
        if (_exerciseTypes.contains(task.taskType)) {
          // Navigate to ExercisePage — it handles submission internally.
          // On return, refresh so completedTaskIds are up-to-date.
          ...
          // Non-exercise tasks (recording, writing, etc.) toggle locally for now.
